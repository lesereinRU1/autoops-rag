...
class AnswerGenerator:
    ...
    def _repair_prompt(
        ...
        return (
            ...
            "若校验提示标识或扩展性措辞没有证据，请删除该标识或措辞，并直接改写所引来源原文；"
            "不得补写通信资源调度、通信块内部机制或设备差异推断。"
    ...
    def generate(
        ...
        *,
        ...
        prompt = (
            ...
            "- 英文标识必须在该事实句引用的同一个来源中逐字出现；只在问题或其他来源中出现时，从事实句删除。\n"
            "- 改写示例：证据只写 BUSY 时不要反复触发 REQ，就写“在 BUSY 时反复触发 REQ 不符合当前证据中的请求触发要求”，不要补写通信资源调度或通信块内部机制。\n"
            "- 改写示例：证据写“多寄存器数值的字顺序由设备实现决定”，就原样表达，不要扩写为设备‘可能’采用某种排列。\n"
