...
Python 3.11、FastAPI、LangGraph、Qdrant、BGE/FastEmbed、BM25、RRF、PyMuPDF、SQLite、Docker Compose、Pytest。
...
| 工具封装 | 统一 `ToolResult`，封装故障码、参数和只读表格行 SQLite 查询，不伪造 provenance |
...
flowchart LR
    ...
    SHADOW --> TOOL["Structured Tool Lookup"]
    TOOL --> RET["Hybrid Retrieval"]
    RET --> D["Qdrant / BGE"]
    RET --> B["BM25"]
...
5. 固定路由执行结构化查询，并进入 Dense + BM25 + RRF + rerank 检索。
...
- `ToolRouter`：为 intent 生成白名单候选工具序列。
- `BoundedQueryPlanner`：最多生成 3 个白名单步骤，带 `max_rounds`、`max_tool_calls` 和 `applied=false`。
- `SQLiteToolbox`：以统一 `ToolResult` 返回内容、证据、provenance、耗时和错误；工具结果不能在没有可靠来源时直接成为最终事实。
...
因此默认 API 不会让 Agentic Router、Planner 或 Iterative Retrieval 接管主流程。即使启用 Planner，当前阶段也只产生 shadow trace。
